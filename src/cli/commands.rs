...
pub fn init_command(name: Option<&str>, dir: Option<&Path>) {
    ...
    println!("Initializing forge managed folder '{}' at {}", folder_name, init_dir.display());
...
pub fn stage_command(files: &[PathBuf], recursive: bool, max_depth: Option<usize>) {
    ...
    println!("Using managed folder '{}' at {}", folder_name, forge_path.display());
    
    if !forge_path.exists() {
        match fs::create_dir_all(&forge_path) {
            Ok(_) => println!("Created forge directory: {}", forge_path.display()),
            Err(e) => {
                eprintln!("Failed to create forge directory {}: {}", forge_path.display(), e);
                return;
            }
        }
    }
    ...
    for file in files {
        ...
        if abs_source.is_dir() {
            ...
            if recursive || max_depth.is_some() {
                ...
                println!("Processing directory: {} (max depth: {})", 
                         abs_source.display(), 
                         if walkdir_depth == usize::MAX { "unlimited".to_string() } else { walkdir_depth.to_string() });
                ...
                for entry in walkdir::WalkDir::new(&abs_source)
                    ...
                    let rel_path = entry.path().strip_prefix(&abs_source)
                    ...
                    if target.exists() {
                        println!("Target already exists in forge folder: {}", target.display());
                        continue;
                    }
                    ...
                    match symlink::create_symlink(entry.path(), &target) {
                        Ok(_) => {
                            println!("Created staging symlink: {} → {}", target.display(), entry.path().display());
                            
                            // Add to database as staged
                            match config::stage_dotfile(entry.path(), &target, None) {
                                Ok(_) => println!("Staged file: {} (use 'link' to make permanent)", entry.path().display()),
                                Err(e) => eprintln!("Failed to stage {}: {}", entry.path().display(), e),
                            }
                        },
                        Err(e) => eprintln!("Failed to create staging symlink for {}: {}", entry.path().display(), e),
                    }
                }
            } else {
                println!("Skipping directory: {} (use --recursive or --depth to include contents)", abs_source.display());
                ...
                if let Some(dirname) = file.file_name() {
                    ...
                    match config::stage_dotfile(&abs_source, &target, None) {
                        Ok(_) => println!("Staged directory: {} (use 'link' to make permanent)", abs_source.display()),
                        Err(e) => eprintln!("Failed to stage directory {}: {}", abs_source.display(), e),
                    }
                } else {
            ...
            if let Some(filename) = file.file_name() {
                ...
                match symlink::create_symlink(&abs_source, &target) {
                    Ok(_) => {
                        println!("Created staging symlink: {} → {}", target.display(), abs_source.display());
                        
                        // Add to database as staged
                        match config::stage_dotfile(&abs_source, &target, None) {
                            Ok(_) => println!("Staged file: {} (use 'link' to make permanent)", abs_source.display()),
                            Err(e) => eprintln!("Failed to stage {}: {}", abs_source.display(), e),
                        }
                    },
                    Err(e) => eprintln!("Failed to create staging symlink for {}: {}", abs_source.display(), e),
                }
            } else {
...
pub fn link_command(files: &[PathBuf]) {
    ...
    let dotfiles = if files.is_empty() {
        ...
        for file in files {
            ...
            if abs_path.is_dir() {
                ...
                if result.is_empty() {
                    eprintln!("No staged files found for directory: {}", abs_path.display());
                }
            } else {
    ...
    for dotfile in dotfiles {
        ...
        if dotfile.target.is_file() || symlink::is_symlink(&dotfile.target) {
            ...
            if let Err(e) = fs::remove_file(&dotfile.target) {
                eprintln!("Failed to remove staging symlink {}: {}", dotfile.target.display(), e);
                error_count += 1;
                continue;
            }
        } else if dotfile.target.is_dir() {
            // For directory targets, just skip removal
            println!("Target is a directory, skipping removal: {}", dotfile.target.display());
        }
        ...
        match fs::copy(&dotfile.source, &dotfile.target) {
            Ok(_) => {
                ...
                if let Err(e) = fs::remove_file(&dotfile.source) {
                    eprintln!("Failed to remove original file {}: {}", dotfile.source.display(), e);
                    error_count += 1;
                    continue;
                }
                
                // Create symlink from original location to forge directory
                match symlink::create_symlink(&dotfile.target, &dotfile.source) {
                    Ok(_) => {
                        println!("Created symlink: {} → {}", dotfile.source.display(), dotfile.target.display());
                    ...
                    Err(e) => {
                        eprintln!("Failed to create symlink {} → {}: {}", 
                            dotfile.source.display(), dotfile.target.display(), e);
                        
                        // Try to restore original file
                        match fs::copy(&dotfile.target, &dotfile.source) {
                            Ok(_) => println!("Restored original file: {}", dotfile.source.display()),
                            Err(e) => eprintln!("Failed to restore original file {}: {}", dotfile.source.display(), e),
                        }
            ...
            Err(e) => {
                eprintln!("Failed to copy file {} to {}: {}", 
                    dotfile.source.display(), dotfile.target.display(), e);
                error_count += 1;
            }
        }
    ...
    println!("\nSymlink creation completed: {} succeeded, {} failed", success_count, error_count);
}
                ...
                if file.file_name().is_some() {
                    ...
                    if symlink::is_symlink(&orig_path) {
                        ...
                        match fs::remove_file(&orig_path) {
                            ...
                            Err(e) => println!("Failed to remove symlink {}: {}", orig_path.display(), e),
                        }
                    } else {
        ...
        match fs::copy(&dotfile.target, &dotfile.source) {
            Ok(_) => {
                ...
                match fs::remove_file(&dotfile.source) {
                    Ok(_) => {
                        println!("Removed symlink and restored file: {}", dotfile.source.display());
                    ...
                    Err(e) => println!("Failed to remove symlink {}: {}", dotfile.source.display(), e),
                }
            }
            Err(e) => println!("Failed to restore file from {}: {}", dotfile.target.display(), e),
        }
    }
}
...
pub fn remove_command(files: &[PathBuf], skip_confirm: bool) {
    ...
    for file in files {
        ...
        if symlink::is_symlink(&dotfile.source) {
            ...
            match fs::copy(&dotfile.target, &dotfile.source) {
                Ok(_) => {
                    // Remove the symlink
                    if let Err(e) = fs::remove_file(&dotfile.source) {
                        println!("Failed to remove symlink {}: {}", dotfile.source.display(), e);
                        continue;
                    }
                }
                Err(e) => {
                    println!("Failed to restore file from {}: {}", dotfile.target.display(), e);
                    continue;
                }
            }
        }
        ...
        match fs::remove_file(&dotfile.target) {
            Ok(_) => {
                println!("Removed file from forge folder: {}", dotfile.target.display());
...
pub fn delete_command(files: &[PathBuf], skip_confirm: bool) {
    ...
    for file in files {
        ...
        if symlink::is_symlink(&dotfile.source) {
            ...
            if let Err(e) = fs::remove_file(&dotfile.source) {
                println!("Failed to remove original file {}: {}", dotfile.source.display(), e);
                println!("Continuing with deletion...");
            }
        }
        ...
        match fs::remove_file(&dotfile.target) {
            Ok(_) => {
                println!("Deleted file from forge folder: {}", dotfile.target.display());
        ...
        println!("File {} has been completely deleted from the system.", dotfile.source.display());
    }
}
                    ...
                    match config::add_dotfile(source, &target, Some(name)) {
                        Ok(_) => {
                            println!("Added to profile '{}': {} → {}", 
                                name, source.display(), target.display());
                            success_count += 1;
                        },
                        Err(e) => {
                            eprintln!("Failed to add to profile '{}': {} → {}: {}", 
                                name, source.display(), target.display(), e);
                            error_count += 1;
                        }
                    }
                }
